<?php get_header(); ?>
<?php // these variables come from advanced custom field plugin images. They are called // carousel, carousel2, carousel3 etc. // they can only be uploaded on the home page.


$image1 = get_field('carousel');
$image2 = get_field('carousel2');
$image3 = get_field('carousel3');
$image4 = get_field('carousel4'); // error handling - the admin needs to add images if they are not present
...
<div class="carousel">
     ...
     <?php if ($image1) { ?> <div class="carousel-cell" style="background-image:url('<?php echo $image1['url'
 ]; ?>')"><h1><?php echo $image1['caption']; ?></h1> 
    <?php echo $image1['description']
    	? '<h1 class="title-small">' . $image1['description'] . '</h1>'
    	: '' ;?>
    </div><?php } ?>
     
      <?php if ($image2 ) { 
        ?> <div class="carousel-cell" style="background-image:url('<?php echo $image2['url']; ?>')"><h1>
      <?php echo $image2['caption']; ?></h1>
       <?php echo $image2['description']
       	? '<h1 class="title-small">' . $image2['description'] . '</h1>'
       	: '' ;?>
    </div><?php } ?> 
     
      <?php if (
      	$image3
      ) { ?>  <div class="carousel-cell" style="background-image:url('<?php echo $image3[
  	'url'
  ]; ?>')"><h1><?php echo $image3['caption']; ?></h1>
       <?php echo $image3['description']
       	? '<h1 class="title-small">' . $image3['description'] . '</h1>'
       	: '' ;?>
    </div><?php } ?>

        <?php if (
        	$image4
        ) { ?>  <div class="carousel-cell" style="background-image:url('<?php echo $image4[
  	'url'
  ]; ?>')"><h1><?php echo $image4['caption']; ?></h1>
       <?php echo $image1['description']
       	? '<h1 class="title-small">' . $image4['description'] . '</h1>'
       	: ''; ?>
    </div><?php } ?>
    ...
    <main class="main-body">
      ...
      <div class="container-secondary">
        <section class="container-news">
          <h2>Latest News</h2>
         <?php // only show first three posts on news section
